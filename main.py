...
def main(cfg: RunConfig) -> None:
    ...
    # ─────────────────────────────────────────────────────────────
    # STEP 9: Tomorrow's Prediction + Multi-Ticker Ranking
    # ─────────────────────────────────────────────────────────────
    ...
    def _predict_ticker(ticker_sym: str) -> dict:
        """Run all models on the last sequence for a given ticker and return probs."""
        from data_loader import download
        from features    import build_features, apply_scale, make_sequences

        try:
            raw_t  = download(ticker_sym, DATA.period)
            df_t   = build_features(raw_t)
            df_t   = df_t.replace([np.inf, -np.inf], np.nan).dropna()

            # Only keep the selected features that exist in this ticker's dataframe
            feats_t = [f for f in FEATURES if f in df_t.columns]
            if len(feats_t) < len(FEATURES) * 0.8:
                logger.warning(f"  Ranking: {ticker_sym} missing too many features — skipped.")
                return {}

            X_t      = scaler.transform(df_t[feats_t].iloc[-SEQ:].values).astype(np.float32)
            seq_t    = torch.from_numpy(X_t).unsqueeze(0).to(DEVICE)
            flat_t   = scaler.transform(df_t[feats_t].iloc[[-1]])

            p = {}
            p['XGBoost'] = float(xgb_model.predict_proba(flat_t)[0][1])
            for mname, mobj in [('LSTM', lstm_model), ('GRU', gru_model),
                                 ('Transformer', tf_model), ('TCN', tcn_model)]:
                p[mname] = predict_single(mobj, seq_t, DEVICE)
            ens_p = ens.predict_stacking({k: np.array([v]) for k, v in p.items()})
            p['Ensemble_Stack'] = float(ens_p[0])
            return p

        except Exception as exc:
            logger.warning(f"  Ranking: {ticker_sym} failed — {exc}")
            return {}

    # ── Single-ticker prediction (always) ────────────────────────
    ...
    # ── Multi-ticker ranking (only when multi-ticker mode is active) ──
    ticker_ranking: dict = {}
    if cfg.multi_ticker and len(DATA.tickers) > 1:
        logger.info(f'\n  {div}')
        logger.info(f'  {_label} RANKING ({_h}d)  --  {len(DATA.tickers)} tickers')
        logger.info(f'  {div}')

        all_tickers = list(DATA.tickers)

        # Primary ticker already predicted — reuse its result
        ticker_ranking[DATA.ticker] = preds_tomorrow.get('Ensemble_Stack', 0.5)

        # Predict all other tickers
        for sym in all_tickers:
            if sym == DATA.ticker:
                continue
            t_probs = _predict_ticker(sym)
            if t_probs:
                ticker_ranking[sym] = t_probs.get('Ensemble_Stack', 0.5)

        # Sort best → worst by Ensemble_Stack probability
        ranked = sorted(ticker_ranking.items(), key=lambda x: x[1], reverse=True)

        logger.info(f'  {"Rank":<6} {"Ticker":<8} {"Signal":<6} {"Prob":>7}  {"Strength":<10}')
        logger.info(f'  {"-"*44}')
        for rank, (sym, prob) in enumerate(ranked, 1):
            medal = {1: '[#1]', 2: '[#2]', 3: '[#3]'}.get(rank, f'[#{rank}]')
            logger.info(
                f'  {medal:<6} {sym:<8} {direction(prob)}  {prob*100:>5.1f}%  [{strength(prob)}]'
            )

        logger.info(f'  {"-"*44}')
        top_pick   = ranked[0][0]
        avoid_pick = ranked[-1][0]
        logger.info(f'  Top pick   : {top_pick}  ({ticker_ranking[top_pick]*100:.1f}%)')
        logger.info(f'  Avoid      : {avoid_pick}  ({ticker_ranking[avoid_pick]*100:.1f}%)')
        logger.info(f'  {div}\n')
    ...
    summary = {
        ...
        'ticker_ranking': {
            sym: {
                ...
                'direction':  direction(prob).strip(),
                'strength':   strength(prob),
            }
            for rank, (sym, prob) in enumerate(
                sorted(ticker_ranking.items(), key=lambda x: x[1], reverse=True), 1
            )
        } if ticker_ranking else {},
        'diagnostics': {
